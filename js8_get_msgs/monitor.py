...
"""
Monitor script for JS8Call data collection and aggregation.
"""
...
colors = {
    ...
    'mycall': '#2ECC71',
    'at': '#5DADE2',
    'snr_supergreen': '#66FF00',
    'snr_green': '#7CD342',
    'snr_yellow': '#FDD835',
    'snr_red': '#F4511E',
    'heartbeat': '#FADBD8',
    'query': '#D6EAF8',
    'table_header_background': '#08389F',
    'table_header_text': '#FFFFFF',
    'link': '#000000',
    'border_bottom_odd': '#DDDDDD',
    'border_bottom_even': '#F3F3F3',
    'border_bottom_last': '#009879',
    'cq': '#FCF3CF',
    'non_zombie_traffic': '#FFFFFF',
    'close': '#D4EFDF'
}
...
.styled-table tbody tr {
  border-bottom: 1px solid {border_bottom_odd};
}
.styled-table tbody tr:nth-of-type(even) {
  background-color: {border_bottom_even};
}
.styled-table tbody tr:last-of-type {
  border-bottom: 2px solid {border_bottom_last};
}
* {
  margin: 0;
  padding: 0;
...
prefixes = [
    ['1A', 'Sov Mil Order of Malta'], ['3A', 'Monaco'], ['3B6', 'Agalega & St. Brandon'], ['3B7', 'Agalega & St. Brandon'],
    ...
    ['FT9Y', 'Antarctica'], ['LU1Z', 'Antarctica'], ['LU2Z', 'Antarctica'], ['LU3Z', 'Antarctica'], ['LU4Z', 'Antarctica'],
    ['LU5Z', 'Antarctica'], ['LU6Z', 'Antarctica'], ['LU7Z', 'Antarctica'], ['LU8Z', 'Antarctica'], ['LU9Z', 'Antarctica'],
...
flags = {
    "Sov Mil Order of Malta": "xx.svg", "Spratly Islands": "xx.svg", "Monaco": "mc.svg", "Agalega & St. Brandon": "xx.svg",
...
def extract_call(call):
    """
    Extract the longest subset of a callsign if it contains '/' characters.

    Args:
        call (str): Callsign.

    Returns:
        str: Extracted callsign.
    """
    s = call.split('/')
...
def call_country(call):
    """
    Determine the country of a callsign based on prefixes.
...
def call_flag(call):
    """
    Determine the flag of a callsign based on the country.
...
def housekeeping_thread(name):
    """
    Thread to perform housekeeping tasks such as removing old data.

    Args:
        name (str): Name of the thread.
    """
    global traffic, stations, grids, max_age
    print(name + ' started...')
    while True:
        time.sleep(60.0)
        now = time.time()
...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    """
    HTTP request handler for the monitor server.
    """
    def do_GET(self):
        """
        Handle GET requests.
        """
        global traffic, stations, grids, colors, mycall
        doc, tag, text = Doc().tagtext()
        if self.path == '/':
            ...
            with grids_lock:
                with stations_lock:
                    with traffic_lock:
                        ...
                        out = {
                            'mycall': mycall,
                            'stations': {uuid.decode('utf-8'): json.loads(stations[uuid]) for uuid in stations.keys()},
                            'traffic': [json.loads(traffic[key]) for key in traffic.keys() if json.loads(traffic[key])['stuff']['type'] == 'RX.DIRECTED']
                        }
    ...
    def do_POST(self):
        ...
        """
        global traffic, stations, grids, calldb, max_age
        ...
        if self.path == '/collect':
            ...
            if j['type'] == 'grids':
                ...
                with traffic_lock:
                    fmcall = extract_call(j['stuff']['params'].get('FROM', ''))
                    j['from_call'] = fmcall if fmcall else False
                    fmname, fminfo, fmaddr = (json.loads(calldb[fmcall]).get(key) for key in ('name', 'info', 'address')) if fmcall in calldb else (False, False, False)
                    fmctry = call_country(fmcall)
                    fmflag = call_flag(fmcall)
                    tocall = extract_call(j['stuff']['params'].get('TO', ''))
                    j['to_call'] = tocall if tocall else False
                    j['to_at'] = j['to_call'][0] == '@' if j['to_call'] else False
                    toname, toinfo, toaddr = (json.loads(calldb[tocall]).get(key) for key in ('name', 'info', 'address')) if tocall in calldb else (False, False, False)
                    toctry = call_country(tocall)
                    toflag = call_flag(tocall)
                    fmgrid = json.loads(grids[fmcall]).get('grid') if fmcall.encode('utf-8') in grids else False
                    j['from_grid'] = fmgrid
                    if fmgrid:
                        lat, lon = mh.to_location(fmgrid[:8]) if len(fmgrid) > 8 else mh.to_location(fmgrid)
                        j['from_lat'], j['from_lon'] = lat, lon
                    else:
                        j['from_lat'], j['from_lon'] = False, False
                    togrid = json.loads(grids[tocall]).get('grid') if tocall.encode('utf-8') in grids else False
                    j['to_grid'] = togrid
                    if togrid:
                        lat, lon = mh.to_location(togrid[:8]) if len(togrid) > 8 else mh.to_location(togrid)
                        j['to_lat'], j['to_lon'] = lat, lon
                    else:
                        j['to_lat'], j['to_lon'] = False, False
                    j.update({
                        'from_info': fminfo, 'to_info': toinfo, 'from_addr': fmaddr, 'to_addr': toaddr,
                        'from_name': fmname, 'to_name': toname, 'from_flag': fmflag, 'to_flag': toflag,
                        'from_country': fmctry, 'to_country': toctry, 'snr': j['stuff']['params']['SNR'],
                        'speed': j['stuff']['params']['SPEED'], 'freq': j['stuff']['params']['FREQ'],
                        'text': bleach.clean(j['stuff']['params']['TEXT'], tags={}),
                        'id': ':'.join([fmcall, tocall, str(j['stuff']['time']), str(j['freq'])])
                    })
...
if __name__ == '__main__':
    ...
    mycall = args.call.upper() if args.call else False
    if mycall:
        print('My call sign: ' + mycall)

    listen = int(args.listen) if args.listen else 8000
    ...
    max_age = int(args.max_age) if args.max_age else 3600
