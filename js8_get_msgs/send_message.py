...
def main():
    """
    Main function to send a directed message via JS8Call.
    """
            ...
            if not rx_queue.empty():
                with rx_lock:
                    ...
                    if rx['type'] == "RX.DIRECTED":
                        if rx['params']['CMD'] == " SNR" and rx['params']['FROM'] == args.call and rx['params']['TO'] == mycall:
                            snr = True
                            done = time.time()
                            if args.verbose:
            ...
            while done > time.time():
                ...
                if not rx_queue.empty():
                    with rx_lock:
                        rx = rx_queue.get()
                        if rx['type'] == "RX.DIRECTED":
                            if rx['params']['CMD'] == " ACK" and rx['params']['FROM'] == args.call and rx['params']['TO'] == mycall:
                                if args.verbose:
