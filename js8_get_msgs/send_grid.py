...
def ll2mh(lat, lon, length):
    ...
    Args:
        lat (float): Latitude.
        lon (float): Longitude.
        length (int): Length of the grid square.
...
def c2v(c):
    """
    Convert a character to its corresponding value.
...
def main():
    """
    Main function to send grid square to APRS.
    """
    ...
    gpsdhost = args.gpsd_host if args.gpsd_host else os.environ.get("GPSDHOST") if args.env else "localhost"
    gpsdport = int(args.gpsd_port) if args.gpsd_port else int(os.environ.get("GPSDPORT")) if args.env else 2947
